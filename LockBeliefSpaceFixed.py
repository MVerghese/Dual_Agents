...
# 		# print(self.config)
# 		rel_config = self.config[:,:,:,self.current_state[action]]
...
class Structured_Agent:
	...
	def dump_graphs(self,graph_filename = 'prior_graphs.npy',cids_filename = 'prior_cids.npy'):
		np.save(graph_filename,self.prior_graphs, allow_pickle = True)
		np.save(cids_filename,self.prior_cids, allow_pickle = True)

	def load_graphs(self,graph_filename = 'prior_graphs.npy',cids_filename = 'prior_cids.npy'):
		self.prior_graphs = np.load(graph_filename, allow_pickle = True)
		self.prior_cids = np.load(cids_filename, allow_pickle = True)

	def distance_metric(self,cids1,cids2):
		counts1 = np.bincount(cids1)
		counts2 = np.bincount(cids2)

		counts1_padded = np.zeros(max(cids1.shape[0],cids2.shape[0]),dtype=int)
		counts2_padded = np.zeros(max(cids1.shape[0],cids2.shape[0]),dtype=int)

		counts1_padded[:counts1.shape[0]] = counts1
		counts2_padded[:counts2.shape[0]] = counts2

		diff = np.sum(np.abs(counts1_padded-counts2_padded))
		return(diff)
	...
	def init_policy(self,num_components,cids,goal_state,use_priors = True, priors_k = 5, prior_entropy = 0.01):
		self.cids = cids
		self.goal_state = goal_state
		self.state_hash = np.zeros(2**num_components)
		self.belief_state = np.zeros((5,2,5,2))
		if use_priors:
			distances = np.zeros(len(self.prior_cids),dtype=int)
			for i in range(distances.shape[0]):
				distances[i] = self.distance_metric(cids,self.prior_cids[i])
			...
			p = 1
			for idx in relevant_graph_idxs:
				weights.append(1.)
				# if distances[idx] == 0:
				# 	weights.append(1.)
				# 	break
				# else:
				# 	weights.append(1/(distances[idx]**p))
			weights = np.array(weights)
			weights /= np.sum(weights)
			# print("Weights:", weights)

			component_interaction_graph = np.zeros((np.max(cids)+1,2,np.max(cids)+1,2))
			
			#For each of the relevant graphs
			for idx in range(weights.shape[0]):
				k = relevant_graph_idxs[idx]
				# print("Prior CIDS:", self.prior_cids[k])
				# print_belief_state(self.prior_graphs[idx],"Single Prior Graph")
				prior_n_cids = np.max(self.prior_cids[k])

				#construct the temporary component interaction graph for this prior dependency graph
				temp_graph = np.zeros((np.max(cids)+1,2,np.max(cids)+1,2))
				temp_graph_counts = np.zeros((prior_n_cids+1,prior_n_cids+1))

				# prior_cid_counts = np.bincount(self.prior_cids[k])
				#For each edge in the prior dependency graph
				for i in range(self.prior_graphs[k].shape[0]):
					for j in range(self.prior_graphs[k].shape[2]):
						# cid_norm_count = prior_cid_counts[self.prior_cids[k][i]]*prior_cid_counts[self.prior_cids[k][j]]
						# print(cid_norm_count)
						if self.prior_cids[k][i] < temp_graph.shape[0] and self.prior_cids[k][j] < temp_graph.shape[2]:
							# print("Dependency graph idxs: ",i,j)
							# print("CI Graph idxs: ",self.prior_cids[k][i],self.prior_cids[k][j])
							# print(self.prior_graphs[k][i,:,j,:])
							
							temp_graph[self.prior_cids[k][i],:,self.prior_cids[k][j],:] += self.prior_graphs[k][i,:,j,:]
							# print(temp_graph[self.prior_cids[k][i],:,self.prior_cids[k][j],:])
							temp_graph_counts[self.prior_cids[k][i],self.prior_cids[k][j]] += 1.

				# print_belief_state(temp_graph,"pre_norm_ci_graph")
				#Normalize component interactions
				for i in range(temp_graph_counts.shape[0]):
					for j in range(temp_graph_counts.shape[1]):
						if temp_graph_counts[i,j] > 0:
							temp_graph[i,:,j,:] /= temp_graph_counts[i,j]

				component_interaction_graph += temp_graph*weights[idx]

			# print_belief_state(component_interaction_graph,"ci_graph")


			for i in range(num_components):
				for j in range(num_components):
					self.belief_state[i,:,j,:] = component_interaction_graph[cids[i],:,cids[j],:]
...
class Structured_Agent_Dist:
	...
	def save_graph(self):
		...
		# aug_dist = np.zeros((self.dists.shape[0],4))
		# aug_dist[:,:2] = self.dists
